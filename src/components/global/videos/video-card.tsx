        ...
        <Loader state={props.processing} className='bg-[#171717] flex justify-center items-center border-[1px] border-[#252525] rounded-xl'>
            <div className='group overflow-hidden cursor-pointer bg-[#171717] relative border-[1px] border-[#252525] flex flex-col rounded-xl'>
                <div className='absolute top-3 right-3 z-50 gap-x-3 hidden group-hover:flex'>
                    <VideoCardMenu videoId={props.id} currentWorkspace={props.workspaceId} currentFolder={props.Folder?.id} currentFolderName={props.Folder?.name} />
                    <CopyLink className="p-0 h-5 hover:bg-transparent bg-[#252525]" videoId={props.id} />
                </div>
                <Link href={`/dashboard/${props.workspaceId}/video/${props.id}`} className='hover:bg-[#252525] transition duration-150 flex flex-col justify-between h-full'>
                    ...
                    <div className='px-5 py-3 flex flex-col z-20'>
                        ...
                        <div className='flex gap-x-2 items-center mt-4'>
                            ...
                            <div>
                                <p className=' text-xs text-[#bdbdbd] capitalize'>{props.User?.firstname} {props.User?.lastname}</p>
