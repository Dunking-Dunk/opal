...
export const useMoveVideos = (videoId: string, currentWorkspace: string, currentFolder: string) => {
    //get state redux
    const { folders } = useAppSelector((state) => state.FolderReducer)
    const { workspaces } = useAppSelector((state) => state.WorkspaceReducer)
    const [invalidate, setInvalidate] = useState<string[][]>([])
    // fetching states
    ...
    const { mutate, isPending } = useMutationData(
        ...
        (data: { folder_id: string; workspace_id: string }) => {
            setInvalidate([['folder-videos', currentFolder], ["workspace-folders", currentWorkspace], ['folder-videos', data.folder_id], ["workspace-folders", data.workspace_id], ['user-videos', currentWorkspace], ['user-videos', data.workspace_id]])
